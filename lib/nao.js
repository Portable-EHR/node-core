...
const logger = log4js.getLogger('nao');
...
DeclareExpectedError(IpSocketError);                                //  Overriden by DeclareFeedOpError() later soon.
...
DeclareExpectedError(StatusError);                                  //  Overriden by DeclareFeedOpError() later soon.
...
const Unpacking = self.Unpacking = function Unpacking(error, msgHead=()=>'', callContext='', errorContext='', body=undefined) {
    ...
    return Object.assign(e,  {
        shortMessage() {                                        //  a closure to use errorContext without callContext
            const {feedOp:{isOfFeed, FeedProvider, isPumpOp}, extraStr} = this;
            const { message, constructor:{name:wrappedErrorName}} = error;
            const server = isOfFeed ? `${FeedProvider} Feed` : 'Backend';
            return `Unpacking${isPumpOp ? ': ' : ` ${server} response: ${errorContext}`}${wrappedErrorName}: ${
                                                  message} :${extraStr}${isOfFeed && !isPumpOp? `\n${msgHead()}` : ''}`;
        }});
};
DeclareExpectedError(Unpacking);                                    //  Overriden by DeclareFeedOpError() later soon.


const BackendError = self.BackendError = function BackendError({msgHead, statusCode, requestStatus}) {
    const {apiVersion, status, message} = requestStatus;
    return ErrorExtender(`Backend${apiVersion ? ` Api[${apiVersion}]` : ''}: [${status}] : ${message}`, BackendError, {
                                backendApiRequestStatus:requestStatus,
                            },()=>`${msgHead()} resulted in (HTTP statusCode [${statusCode}]) :\n`);
};
DeclareExpectedError(BackendError);                                 //  Overriden by DeclareFeedOpError() later soon.
...
const EWebStatusCode = (f=>{f.prototype=new Enum(f); return new f({});})(function EWebStatusCode({
    ...
    UnsuportedMediaType=(f=>f(f))(function UnsuportedMediaType(f,i=415){ return EItem(EWebStatusCode, f,i); }),
    ServerError  =(f=>f(f))(function ServerError(f,   i=500) { return EItem(EWebStatusCode, f, i); }),
                                            ...
                                            Gone, UnsuportedMediaType, ServerError})); });
...
class WebRequest {
    ...
    async addJwtHeaders(nodeOptions) {
        const { verbose } = this;
        try {
            Object.assign(nodeOptions.headers, {
    ...
    async getJwt(verbose) {
        ...
        //  using adapter.validateAndLinkEndpointCredentials(), or by some other means (e.g. implicitely in selfServer).
        try {
            ...
            logger.error(`getJwt() : Could not obtain jwt from server : ${e.message}`);
            throw e;
        }
    }
    ...
    async performRequest({nodeOptions:options, _bodyToSendObCandidate={}}) {
        ...
        const performOneRequest = async () => new Promise(async (fulfill, reject) => {
            ...
            if (options.timeout) {
                ...
                //  These are the two events already unconditionnaly handled here, above. For error reporting purpose.
                //
        ...
        if (maxAttempts < 0) {      //  If maxAttemps is negative, return the closures performOneRequest, with msgHead
        ...
        //  Cycle trough as many attempts of performOneRequest as specified by maxAttempts.
        ...
        for (let attemptCnt = 0; attemptCnt++ < maxAttempts; ) {    //  Note the post-increment! Readies the test below.
            try {
                ...
                if (verbose) {
                    const xtra = (feedItem => feedItem ? feedItem._feedOp.srcAndDstItemStrs.join('') : ''
...
self.SelfWebRequestMethods = (endpoint, {apiUsers}) => {
       ...
       StaticOptions : {
        ...
        obtainJwt: async (verbose) => {
            const {body/*, statusCode2XX, msgHead*/} = await Post({
                //  SMALL HACK : To prevent adding auth headers, Post('/login') apiUserName argument must always get
                //  an apiUser which .method is neither 'bearer' nor 'custom'. The '_selfServerDummyLogin. apiUser is
                apiUserName: '_selfServerDummyLogin',               //  hard-coded in ApiUsers just for that purpose.
                path: '/login',
                parameters: endpoint._credentials
            }, {verbose});                                                                              //  May throw !
    ...
    const webRequestEndpointOptions = ({timeoutInMs=1000*150, maxAttempts=1, verbose=undefined, ...extraOptions}={}) => ({
    ...
    const addBearerHeaders = async (apiUser,nodeOptions) => {
        ...
        //  Change the restServer enpoint ._credentials on every call so that endpoint.addJwtHeaders() could be used.
        Object.defineProperty(endpoint, '_credentials', {configurable:true, value: creds});
        await webRequest.addJwtHeaders(nodeOptions);
    };

    /**
     *
     * @param {string} apiUserName: name of the apiUser pretending to perform this selfWebRequest
     * @param {string} path that the selfWebRequest is addressed to.
     * @param {Object} parameters sent in body to the selfWebRequest url, defaults to {}
     * @param {Boolean|undefined} verbose: if undefined (default), endpoint.verbose is used
     * @param {Object} extraHeaders, set apart from extraOptions, for easy merging with apiUser.optionsHeaders...
     * @param {...Object} timeOutInMs_maxAttempts_extraOptions: optional arguments to override selfWebRequest default
     * @returns {Promise<{body:string, statusCode2XX:number, msgHead:(function():string)}|(function():Promise<{body:string, statusCode2XX:number, msgHead:(function():string)}>)>}
     * @constructor
     */
    const Post = async ({apiUserName, path, parameters={}},
                        {verbose=undefined, headers:extraHeaders={}, ...timeOutInMs_maxAttempts_extraOptions}={}) => {

        const apiUser = apiUsers.getUser(apiUserName);
        if (!apiUser) throw Error(`No ApiUser found with name [${apiUserName}].`);

        const { headers:authHeaders } = apiUser.allowsCustomAuthMethod  ?  apiUser.optionsHeadersApikeyAndGuid
            // : apiUser.allowsBearerAuthMethod  ?  apiUser.optionsHeadersBearer //  Covered below.
            :  {headers: {}};

        const nodeOptions = webRequest.nodeOptions(eWebPOST, path, webRequestEndpointOptions({verbose,
            ...timeOutInMs_maxAttempts_extraOptions, headers: { ...authHeaders,         //  order matters: extraHeaders
                ...extraHeaders }}));   //     may override authHeaders

        if ( apiUser.allowsBearerAuthMethod) await addBearerHeaders(apiUser, nodeOptions);  //  conditional auth part

        return webRequest.performRequest({nodeOptions, _bodyToSendObCandidate:parameters});
    };
...
//region BackendWebRequestMethods

const BackendStatusErrorBodyProto = {
    toString() { return this.body; },
    expose() {
        const {body, statusCode} =this;
        return statusCode===500 ?
                body :
            400 <= statusCode && statusCode < 500 ?
                ((htmlBody='')=>htmlBody.replace(/\n/g,''))(body.match(/<body>([^]*)<\/body>/m)[1]) :
                //  default
                '';
    },
};
function BackendStatusErrorBody(statusCode_body) {const o=Object.create(BackendStatusErrorBodyProto); Object.assign(o, statusCode_body); return o;}
BackendStatusErrorBody.chainProto();

const BackendRequestProto = {};
function BackendRequest(apiKey, language, appAlias, appGuid, appVersion,
                        {command, parameters, trackingId=undefined, ...restOfBackendRequest}={}) {
    const o = Object.create(BackendRequestProto);
    Object.assign(o, {
        command,
        parameters,
        trackingId: undefined === trackingId ? guid.create().toString() : trackingId,
        apiKey,
        language,
        appAlias,
        appGuid,
        appVersion,
        ...restOfBackendRequest                     // Might override previous default properties, and add some more
    });
    return o;
}
BackendRequest.chainProto();

self.BackendWebRequestMethods = (endpoint, {language}, {appAlias, appGuid, appVersion}) => {
    ...
    const webRequest = new WebRequest({ endpoint,
        ...
        //  WebRequest via its .nodeOptions(). If it includes a "headers: { ... }"  property, the sub properties
        //  of that headers object will be Object.assign()-ed to the existing options.headers object property.
        StaticOptions: {StatusErrorBody:BackendStatusErrorBody, headers: {}},
        //  No obtainJwt() defined: webRequesting the backend do not use Jwt for now.
    });

    //  the default _webRequest endpointOptions.
    const webRequestEndpointOptions = ({timeoutInMs=1000*50, maxAttempts = 1, verbose = undefined, ...extraOptions} = {}) => ({
        timeoutInMs, maxAttempts, verbose, ...extraOptions
    });

    /**
     *
     * @param {string} path that the backendRequest is addressed to.
     * @param {string} backendRequest command
     * @param {Object} parameters associated with backendRequest command
     * @param {string|undefined} trackingId if undefined (default), guid.create.toString() is used.
     * @param {...Object} restOfBackendRequest
     * @param {Boolean|undefined} verbose: if undefined (default), endpoint.verbose is used
     * @param {...Object} timeOutInMs_maxAttempts_extraOptions: optional arguments to augment backendRequest default
     * @returns {Promise<{body:string, statusCode2XX:number, msgHead:(function():string)}|(function():Promise<{body:string, statusCode2XX:number, msgHead:(function():string)}>)>}
     * @constructor
     */
    const Post = ({path, backendRequest:{command, parameters, trackingId = undefined, ...restOfBackendRequest
    }}, { verbose = undefined, ...timeOutInMs_maxAttempts_extraOptions} = {}) => {

        const nodeOptions = webRequest.nodeOptions(eWebPOST, path, webRequestEndpointOptions({verbose, ...timeOutInMs_maxAttempts_extraOptions}));

        const _bodyToSendObCandidate = BackendRequest(apiKey, language, appAlias, appGuid, appVersion,
                                                    {command, parameters, trackingId, ...restOfBackendRequest});

        //  auth already in backendRequest body, to be moved in option.header ?
        return webRequest.performRequest({nodeOptions, _bodyToSendObCandidate});
    };

    return Object.freeze({ Post, });                                        //  no jwt : no ...webRequest.jwtMethods,
};
...
self.FeedHubWebRequestMethods = (endpoint) => {
    ...
    const webRequest = new WebRequest({ endpoint,
        ...
        obtainJwt: async (verbose) => {
            const {body/*, statusCode2XX, msgHead*/} = await Post({
                path: '/login',
                postBody: endpoint._credentials,
                requiresJwt: false
            }, {verbose});                                                                              //  May throw !
            logger.debug(`FeedHubWebRequestMethods' WebRequest obtainJwt(): [${body}]`);
            return Jwt(JSON.parse(body).responseContent.token);                                 //  unlikely to throw.
        },
    });

    const webRequestEndpointOptions = ({timeoutInMs=1000*40, maxAttempts=1, verbose=undefined, ...extraOptions}={}) => ({
        timeoutInMs, maxAttempts, verbose, ...extraOptions});

    const Post = async ({path, postBody, requiresJwt=true}, {verbose, ...timeOutInMs_maxAttempts_extraOptions}) => {
        ...
        return await webRequest.performRequest({nodeOptions, _bodyToSendObCandidate:postBody});
    };

    const Get = async ({path, params, requiresJwt=true}, {verbose, ...timeOutInMs_maxAttempts_extraOptions}) => {
