...
const FeedError = self.FeedError = function FeedError({msgHead, statusCode, requestStatus}) {
    const {status, message} = requestStatus;
    return ErrorExtender(`Feed: [${status}] : ${message}`, FeedError, {
        feedApiRequestStatus:requestStatus,
    },()=>`${msgHead()} resulted in (HTTP statusCode [${statusCode}]) :\n`);
};
...
function BackendRequest(apiKey, language, appAlias, appGuid, appVersion,
    ...
    Object.assign(o, {
        command,
        parameters,
        trackingId: undefined === trackingId ? uuidv1() : trackingId,
        apiKey,
        language,
        appAlias,
        appGuid,
