...
const logger       = log4js.getLogger('config.auth');

const {EItem, Enum, emptyField} = require('./utils.js');
...
class ApiUser {
    constructor({password, role, method, apiKey, guid, feeds}, username, apiUsers) {
    ...
    allowsAccessToFeed(srcFeed) {
        return (this._feeds.has('*')  ||  this._feeds.has(srcFeed.id));
    }
