...
const tableLineReservedNameSet= new Set([
    'primary',
    'key',
    'index',
    'fulltext',
...
const parseTableSchema = (tableSchemaSrcOb) =>                  //  tableName     table_def
    tableSchemaSrcOb['Create Table'].replace(/(^\s*CREATE\s+TABLE\s+\`?\w+\`?\s+\()([^]+?)(\).*$)/i,'$2')
            .split(',\n')   // split table_def by line:  ',' would catch the enum(,,,), set(,,), float(,) etc.
            .filter(                                                    //  get rid of the KEY, CONSTRAINT, etc...
                    line => ! tableLineReservedNameSet.has(             //  and only keep the column definitions
...
self.parseTableSchema = parseTableSchema;
