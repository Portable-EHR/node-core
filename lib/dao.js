 ...
 */
const doInTransaction = async (transactionScript= async (transaction)=>{}) => {
    ...
    await (new Promise( (fulfill, reject) => {      //  no try-catch: Error on beginTransaction() just throws.
            connection.beginTransaction(e => {  //  just a convenience function executing SQL command START TRANSACTION.
                if (e) reject(e);
                else fulfill();
            });
    }));
    ...
    try {
        await transactionScript(transaction);       //  any e thrown inside transactionScript causes a rollback() below.

        await (new Promise( (fulfill, reject) => {
            connection.commit(e => {                    //  convenience function executing SQL command COMMIT.
                if (e) connection.rollback(() => {      //  convenience function executing SQL command ROLLBACK.
                        reject(e);                                  //  don't double throw on rollback error.
                    });
                else fulfill();
            });
        }));
        ...
        await (new Promise( (fulfill, reject) => {
            connection.rollback(() => {      //  convenience function executing SQL command ROLLBACK.
                fulfill();                                  //  don't double throw on rollback error.
            });
        }));
        throw e
    }
