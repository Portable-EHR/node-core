    ...
    const schema = tableSchemaSrcOb['Create Table'].replace(/(^\s*CREATE\s+TABLE\s+`?\w+`?\s+\()([^]+?)(\).*$)/i,'$2')
                    ...
                    line => {                                           //  get rid of the KEY, CONSTRAINT, etc...
                        if ( mySqlTableLineReservedNameSet.has(         //  and only keep the column definitions
                            ...
                            if (fkMatch) {
                                ...
                                const foreignKeyCols= foreignKeysStr.split(',')
                                                                    .map(
                                ...
                                const fkColName = (foreignKeyCols.length > 1)  ?  foreignKeyCols  :  foreignKeyCols[0];
                                ...
                                const colNames = colNamesStr.split(',')
                                                            .map(
                                                                 colName =>
                                                                            colName.match(/^\s*`?([^`]+)`?/)[1]);
                                                        //  either colName array   or   single colName string.
                                primaryKey = (colNames.length > 1)   ?   colNames   :   colNames[0];
                            }
        ...
        return Object.assign(
            schema.map(field =>   //  add the .fkTableName and .fkColName properties to a uni-column foreign key field.
                                (fkProps =>  undefined === fkProps  ?  field
                                                                    :  Object.assign(field, fkProps)
                                )(foreignKeyMapOb[field.name])),
                         //  e.g. foreignKeyMapOb['practitioner_id'] = { fkTableName:'Practitioner', fkColName:'id' }

            { primaryKey, foreignKeys });   //  attach .primaryKey and .foreignKeys props to column/field array.
    };
