...
const parseMySqlTableSchema = (tableSchemaSrcOb) => {
    ...
    const schema = tableSchemaSrcOb['Create Table'].replace(/(^\s*CREATE\s+TABLE\s+`?\w+`?\s+\()([^]+?)(\).*$)/i,'$2')
                    ...
                    line => {                                           //  get rid of the KEY, CONSTRAINT, etc...
                        if ( mySqlTableLineReservedNameSet.has(         //  and only keep the column definitions
                            ...
                            if (fkMatch) {
                                ...
                                const [ /*whole*/,  colNamesStr,   foreignTableName, foreignKeysStr] = fkMatch;

                                //  A foreign key can be defined over multiple columns, so instead of colNamesStr
                                //  being "`practitioner_id`" here, it could have been "`col_a`,`col_b`".
                                const colNames = colNamesStr.split(',')
                                                            .map(
                                                                 colName =>
                                                                            colName.match(/^\s*`?([^`]+)`?/)[1]);
                                //  foreignKeys can be defined over multiple columns too, mirroring colNames
                                const foreignKeys=foreignKeysStr.split(',')
                                                                .map(
                                                                    fKeyStr =>  //  Drop the rest: [(length)] [ASC | DESC]
                                                                                fKeyStr.match(/^\s*`?([^`]+)`?/)[1]);

                                                                 //  either colName array   or   single colName string.
                                const fkOwnColName =  (colNames.length > 1)   ?   colNames   :   colNames[0];

                                const fkTableName  = foreignTableName.match(/^\s*`?([^`]+)`?/)[1];
                                                             //  either foreignColName array   or   single foreignColName string.
                                const fkColName    = (foreignKeys.length > 1)   ?   foreignKeys   :   foreignKeys[0];

                                foreignKeys.push({fkOwnColName, fkTableName, fkColName});

                                if (1 === colNames.length) {        //  if the foreign key is over a single column,
                                    // e.g.   ['practitioner_id'] = { fkTableName:'Practitioner', fkColName:'id' }
                                    foreignKeyMapOb[fkOwnColName] = { fkTableName, fkColName }
                                }
                            }
        ...
        schema.foreignKeys = foreignKeys;                   //  attach the foreignKeys array as property of schemaTab
        return schema.map(field =>      //  add the .fkTableName and .fkColName props to a uni-column foreign key field.
                                   (fkOfFieldName =>  undefined === fkOfFieldName  ?  field
                                                                                   :  Object.assign(field, fkOfFieldName)
                                   )(foreignKeyMapOb[field.name]));
    };                      //  e.g. foreignKeyMapOb['practitioner_id'] = { fkTableName:'Practitioner', fkColName:'id' }
