...
class NodeConfig {
    ...
    constructor(srcJsOb, node) {
        ...
        const {launchParams: {environment, application, netFlavor:netFlavorAlias, feedFlavor:feedFlavorAlias } } = node;
        ...
        Object.defineProperty(this, '_netFlavor',   {value: netFlavorAlias});
        Object.defineProperty(this, '_feedFlavor',  {value: feedFlavorAlias});
        ...
        const selfCliWsServer = selfCliServers && selfCliServers[selfCliWsServerId];
        if (selfCliWsServerId) {
            if (undefined === selfCliWsServer) {
                configLogger().bailOut(`netFlavor [${netFlavorAlias}] .selfCliServer: "${selfCliWsServerId}" definition not found in .selfCliServers {} section of config file [${configFQN}].`);
            }
        ...
        if (selfWsServerId) {
            if (undefined === selfWsServer) {
                configLogger().bailOut(`netFlavor [${netFlavorAlias}] .selfWsServer: "${selfWsServerId}" definition not found in .selfWsServers {} section of config file [${configFQN}].`);
            }
