...
const { commentsLitOb, makeDirIfNeeded, buildFromFile, ExpectedError } = require('./utils');
...
function DatabaseConfig({endpoint, user, password, database, debug, ..._rest}={}) {
    const o = Object.create(DatabaseConfigProto);
    Object.assign(o, {endpoint:DbEndpoint(endpoint), user, password, database, debug},
                    commentsLitOb(_rest));
    return o;
}
...
class NodeConfig {
    ...
    constructor(srcJsOb, node) {
        ...
        const { language, nodeName, selfRestServers, selfCliWsServers, selfWsServers, databaseConfigs, msDatabaseConfigs,
                                    apiUsers, credentials, netFlavors } = srcJsOb;
        Object.assign(this, {language, nodeName, selfRestServers, selfCliWsServers, selfWsServers,
                                    databaseConfigs, msDatabaseConfigs, apiUsers, credentials, netFlavors});
        ...
        if (undefined === netFlavor) {
            return configLogger().bailOut(`launchParam (n)et flavor [${netFlavorAlias}] not found in .netFlavors {} section of config file [${configFQN}].`);
        }
        ...
        if (undefined === selfRestServer) {
            return configLogger().bailOut(`netFlavor [${netFlavorAlias}] .selfRestServer: "${netFlavor.selfRestServer}" definition not found in .selfRestServers {} section of config file [${configFQN}].`);
        }
        ...
        if (selfCliWsServerAlias) {
            if (undefined === selfCliWsServer) {
                return configLogger().bailOut(`netFlavor [${netFlavorAlias}] .selfCliWsServer: "${selfCliWsServerAlias}" definition not found in .selfCliWsServers {} section of config file [${configFQN}].`);
            }
        ...
        if (selfWsServerAlias) {
            if (undefined === selfWsServer) {
                return configLogger().bailOut(`netFlavor [${netFlavorAlias}] .selfWsServer: "${selfWsServerAlias}" definition not found in .selfWsServers {} section of config file [${configFQN}].`);
            }
        ...
        if (undefined === databaseConfig) {
            return configLogger().bailOut(`netFlavor [${netFlavorAlias}] .databaseConfig: "${netFlavor.databaseConfig}" definition not found in .databaseConfigs {} section of config file [${configFQN}].`);
        }
        Object.defineProperty(this, '_databaseConfig', {configurable:true, value: DatabaseConfig(databaseConfig)});

        const msDatabaseConfig = this._msDatabaseConfig;
        if (msDatabaseConfigs && this.msDatabaseConfigAlias  &&  undefined === msDatabaseConfig) {
            return configLogger().bailOut(`netFlavor [${netFlavorAlias}] .msDatabaseConfig: "${netFlavor.msDatabaseConfig}" definition not found in .msDatabaseConfigs {} section of config file [${configFQN}].`);
        }
        Object.defineProperty(this, '_msDatabaseConfig', {configurable:true, value: msDatabaseConfig    });
...
//  getFeedFlavor() has different versions in feedHub and feedCore.
    ...
    get _msDatabaseConfigAlias() { return this.netFlavor.msDatabaseConfig; }                                        //  overridden with instance value prop in constructor.
    get msDatabaseConfigAlias() { return this._msDatabaseConfigAlias; }

    get _msDatabaseConfig() { return this.msDatabaseConfigs && this.msDatabaseConfigs[this._msDatabaseConfigAlias]; }   //  overridden with instance value prop in constructor.
    get msDatabaseConfig() { return this._msDatabaseConfig; }
...
self.nodeConfig = (node, NodeConfig) => {
    ...
    try {
        ...
        return buildFromFile(configLogger, configFullFilename, NodeConfig, [node], true);
    }
    finally {
...
self.nodeReloadConfig = (parallelNode, NodeConfig, logger) => {
    ...
    try {
        const configFullFilename = launchParams.configFQN;
        // Clear required cache for configuration file
        delete require.cache[require.resolve(configFullFilename)];
        return buildFromFile(configLogger, configFullFilename, NodeConfig, [parallelNode], true);
    }
    finally {
        delete configLogger.configLog;      //  configLogger is node, really.
        delete configLogger.bailOut;
