...
const logger = log4js.getLogger('nao.self');

const {niceJSON, Cycling}  = require('./utils.js');
const { FeedHubApiResponse, } = require('./api');
const { Unpacking } = require('./feed.core');
const {web:{Post:selfServerPost}}  = require('../../lib/node').config.selfRestServer;
...
const sendOnePostToSelf = self.sendOnePostToSelf = async ({path, requestObject, apiUserName="sysops"},
    ...
    try {
        ...
        return FeedHubApiResponse({status, message, responseContent});
    }
    catch (e) {
...
self.filterFromSrcBundle = ({pullName, path, dispensaryId, fileName, resultsFilter, cycleInterval=0.25,
...
self.runSelfServerUnitTest = async (dispensaryId, verbose=false) => {
    const pingReq = {
        dispensaryId,
        command: "ping",
        trackingId: "5c40816c-d544-461c-8df8-d39237ffb807",
    ...
    const selfPost = async (path, requestObject, verbose = false, timeOutInMs = 1000 * 300) => {
        logger.info(`runSelfServerUnitTest : sendOnePostToSelf('${path}', dispensaryId [${requestObject.dispensaryId}], command: '${requestObject.command
