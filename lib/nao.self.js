...
const { config, isFeedHub, } = require(process.env.PEHR_NODE_CWD+(process.env.PEHR_NODE_LIB_NODE || '/lib/node'));
const selfServerPost = config.selfRestServer.web.Post;
const NodeApiResponse =  isFeedHub  ?  ExtractFeedHubApiResponse  :  ExtractFeedApiResponse;
    ...
    const pull = Object.defineProperties(new Cycling({cycleInterval,
        doOneAction: async function () {
            const {responseContent:{hasMore, results}} = await sendOnePostToSelf({path,
                requestObject: Object.assign(isFeedHub ? {dispensaryId:feedAlias} : {feedAlias}, {
                    command: "pullBundle",
