...
const logger = log4js.getLogger('config.nao');
...
class Adapter {
     ...
     * @param {Feed} feed
     */
    constructor({name, version, verbose, endpoint, bundleChunkIntervalInMs, cachingEnabled}={}, feed) {   // config Object typically
     ...
     */
    validateAndLinkEndpointCredentials(allCredentials, configLogger, errMsgs) {
        ...
        if ('string' !== typeof credentialsName  &&  feedProvider.isCredentialsMandatory) {
            ...
            errMsgs.push(`${this.feed.fullTag} endpoint.credentials [${credentialsName
                                                }] doesn't match any of those defined in feeds config "credentials".`);
        }
    }
    ...
    bind() {    //  bind sapi and endpoint WebRequestMethods
        ...
        //  Feeds that are *enabled*. Because of feed.enabledByLink, a Feed is know for sure to be enabled only _after_
        ...
        Object.defineProperty(this, "_backendWeb", {configurable:true, value: feed.backendpoint._web});
