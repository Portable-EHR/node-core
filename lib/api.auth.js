...
const { EFeedHubRequestStatus, FeedHubApiResponse, EFeedRequestStatus, FeedApiResponse } = require('./api');
 ...
 */
const authResponse = message => NodeApiResponse({status:eNodeRequestStatusAuth, message});
/**
 *
...
self.createJwt = apiUser => {
    ...
    // jwt.setExpiration(dateAdd.minute(1));                    //  Make the jwt expire 1 minute from now.
    jwt.setExpiration(dateAdd.hour(12));                    //  Make the jwt expire 12 hours from now.
    jwt.setNotBefore();                                     //  Make the jwt enabled for validation now.
...
const authenticateAndAllowUser = self.authenticateAndAllowUser = (reqHeaders, isRoleAllowed) => {
    ...
    if ( ! isRoleAllowed(apiUser)) throw AuthError(NodeApiResponse({
 ...
 * @returns {Feed}
 */
const getFeed = ({feedId, Feed}) => {
    /**
     * @type {Feeds}
     */
    const feeds =  node[Feed.feedsname];
    const feed = feeds && feeds.enabled[feedId];

    if (!feed) throw AuthError(NodeApiResponse({
        status  : eNodeRequestStatusNotFound,
        message : `${Feed.Feedname} [${feedId}].`
    }));
    return feed;
};
 ...
 * @param {object} feedIdAndFeed
 * @param {function(ApiUser):boolean} isRoleAllowed
 * @returns {Feed}
 */
self.authorizeApiRequest = (reqHeaders, feedIdAndFeed, isRoleAllowed) => {
    const apiUser = authenticateAndAllowUser(reqHeaders, isRoleAllowed);
    const feed = getFeed(feedIdAndFeed);                            //  throws if no enabled Feed with feedID in Feeds.

    if ( ! apiUser.allowsAccessToFeed(feed)) throw AuthError(NodeApiResponse({
        status  : eNodeRequestStatusAccess,
        message : `User [${apiUser.username}] is denied access to ${feed.Feedname} [${feed.id}] resource.`
    }));
