 ...
 */
const dbInsert = async (insertQueryString, filterResults=(results, reject) => {
                            // console.log(`MS-SQL dbInsert(): results :`, results);
                            const { rowsAffected:[rowsAffected,], recordset }= results;
                            if (rowsAffected > 0) return recordset[0].id;
                            reject(NoRow(`MS-SQL dbInsert() : rowsAffected [${rowsAffected
...
class DbTransaction {
    ...
    async dbInsert(insertQueryString, filterResults=(results, reject) => {
                            // console.log(`MS-SQL trans.dbInsert(): results :`, results);
                            const { rowsAffected:[rowsAffected,], recordset } = results;
                            if (rowsAffected > 0) return recordset[0].id;
                            reject(NoRow(`MS-SQL trans.dbInsert() : rowsAffected [${rowsAffected
                                                }] < 1, performing DB query :\n"${insertQueryString}"`, results)); }) {
