...
const { expectedErrorTransportStatus, expectedBackendErrorMaintenanceStatus,
        expectedErrorLogMessage, } = require('./feed.core');
...
function BackendWtf({createdOn, occuredOn, dispensaryId, source, wtf:message}) {
    const o = Object.create(BackendWtfProto);
    Object.assign(o,{createdOn, occuredOn, dispensaryId, source, message});
    return o;
}
...
class WTFpump extends Cycling {
    constructor(WtfRecord, nao) {
        ...
        this.pushWtfs = nao.pushWtfs;      //  arrow function
    }
    ...
    deleteOldRecords() {                                    //  Not async method, the cron job running this don't wait.
        ...
        if (purgeAfterInDays > 0) {
            ...
            require('./nodeCore/lib/dao').dbDelete("DELETE from WTF WHERE created_on < ?", [then],
    ...
    async doOneCycleAction() {
        ...
        if (wtfRecs && wtfRecs.length > 0) {
            ...
            try {
                ...
                if (e.isExpected) {
                    logger.error(logMsg + expectedErrorLogMessage(e, this.verbose));

                    if (expectedBackendErrorMaintenanceStatus(e)) {
                        await this.postpone();
                    }
                }
                else {
                ...
                if (expectedErrorTransportStatus(e)){
                    await this.postpone();
                }
            }
        }
