...
class Node {
    constructor (nodeName, {appAlias, appGuid, appVersion},
        ...
        Object.defineProperty(this, "_nodecore_initialize", {configurable:true, value: async function(app, logger) {
                ...
                const {database} = pool.config.connectionConfig;
                try {
                    const result = await dao.fetchFromDb(`SELECT db_version FROM Configuration`);
                    // noinspection JSUnresolvedVariable
                    const res = result[0].db_version;
                    if (res) {
                        logger.info(`[DB VERSION]                 : ${res}`);
                    }
                    else logger.error(`Could not extract db_version from ${database} database.`)
                }  catch (e) { logger.error(`querying db_version from DB ${database}.Configuration :\n` + dbMsg(e)); }

                try {
                    const tableNames = await dao.fetchFromDb(('SELECT table_name FROM information_schema.tables' +
                    ...
                    for (let {table_name} of tableNames) {      // can't use  tableNames.reduce() with async / await.
                        tableSchema[table_name] = (await dao.fetchFromDb(`SHOW CREATE TABLE ${table_name}`))[0];
                    }

                    logger.info(`[DB SCHEMA]                  : ${database} :${' '.repeat((n => n < 0 ? 0 : n)(37-database.length))} read !`);
                }  catch (e) { logger.error(`querying DB ${database} Tables Schema :\n` + dbMsg(e)); }
    ...
    get srcProcessResourcesPath()  { return process.cwd()+`/process/resources/${this.environment}`; }
    get srcInstanceResourcesPath() { return process.cwd()+`/instance/resources/${this.environment}`; }
}
self.Node = Node;
