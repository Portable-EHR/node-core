...
const { NoRow, EDbJsType  } = require('./dao');

const self = module.exports;


Object.assign(self, { NoRow, EDbJsType, });     // Attach to my-dao too.
...
const pool = (node => {
    const {endpoint:{host, port}, user, password, database, debug} = node.config.databaseConfig;
    ...
    try {
        ...
        self.poolConnectionTestPromise = new Promise( (fulfill, reject) => {
            pool.getConnection((e, connection)=>{
                if (e) {
                    e.bailOutMsg = `Could not obtain connection from pool for MySQL database [${database}] :`;
                    reject(e);
                } else {
                    connection.query('select 1+1 as answer', (e, results)=> {
                        connection.release();
                        if (e) {
                            e.bailOutMsg = `Could not use ${repr(connection)} with test query :`;
                            reject(e);
                        }
                        const {answer} = results[0];
                        logger.info(`MySQL Database connection pool to [${database}] of [${host}:${port}] configured and functional! [1+1=${answer}]`);
                        fulfill(pool);
                    });
                }
            });
        ...
        throw Error(`Could not create connection pool to MY-DB with conf : ${niceJSON(poolConf)}\n${e.message}`);
...
const getPoolConnectionAsync = () => new Promise( (fulfill, reject) => {
    const errMsg = "Unable to get new MySQL DB connection from pool :\n";
    try {
        pool.getConnection((e, connection)=>{
            if (e) {
 ...
 */
const dbInsert = async (insertQueryString, args=[], filterResults=(results, reject) => {
                                                    // console.log(`MY-SQL dbInsert(): results :`, results);
                                                    const {insertId} = results;
                                                    if (insertId > 0) return insertId;
                                                    reject(NoRow(`MySQL dbInsert : insertId [${insertId
                                                                    }] < 1, performing MY-DB query :\n"${
...
const dbUpdate = async (updateQueryString, args=[], filterResults=(results, reject) => {
                            ...
                            reject(NoRow(`MySQL dbUpdate : changedRows [${changedRows}] < 1, performing MY-DB query :\n"${
...
const dbDelete = async (deleteQueryString, args=[], filterResults=(results, reject)=>{
                            ...
                            reject(NoRow(`MySQL dbDelete : affectedRows [${affectedRows}] < 1, performing MY-DB query :\n"${
...
//         reject( NoRow(`dbGetRecord : no record selected, performing MY-DB query :\n"${selectQueryString}" [${args}]`, results) );
//     });
...
class DbTransaction {
    ...
    async dbInsert(insertQueryString, args=[], filterResults=(results, reject) => {
                                                    // console.log(`MY-SQL trans.dbInsert(): results :`, results);
                                                    const { insertId } = results;
                                                    if (insertId > 0) return insertId;
                                                    reject(NoRow(`MySQL trans.dbInsert : insertId [${insertId
                                                                    }] < 1, performing MY-DB query :\n"${
                                                                    insertQueryString}" [${args}]`, results)); }) {
    ...
    async dbUpdate(updateQueryString, args=[], filterResults=(results, reject) => {
                        const {changedRows} = results;
                        if (changedRows > 0) return changedRows;
                        reject(NoRow(`MySQL trans.dbUpdate : changedRows [${changedRows}] < 1, performing MY-DB query :\n"${
                                                                       updateQueryString}" [${args}]`, results)); }) {
        const { _connection } = this;
        return await(new Promise( (fulfill, reject) => {
    ...
    async dbDelete(deleteQueryString, args=[], filterResults=(results, reject)=>{
                        const {affectedRows} = results;
                        if (affectedRows > 0) return affectedRows;
                        reject(NoRow(`MySQL trans.dbDelete : affectedRows [${affectedRows}] < 1, performing MY-DB query :\n"${
                                        deleteQueryString}" [${args}]`, results));}) {
        const { _connection } = this;
        return await (new Promise( (fulfill, reject) => {
...
//              await trans.dbInsert('blah bla')
//              const data = await trans.fetchFromDb('blah bla');
//              await trans.dbUpdate('blah bla')
//           });
//      }
//      catch (e) {
...
const mySqlTableColumnTypeMapOb = {
    ...
    tinytext:   eStringDbJsType,
    mediumtext: eStringDbJsType,
    longtext:   eStringDbJsType,
    blob:       eStringDbJsType,
    tinyblob:   eStringDbJsType,
    mediumblob: eStringDbJsType,
    longblob:   eStringDbJsType,
...
const parseMySqlTableSchema = (tableSchemaSrcOb) => {
    ...
    const foreignKeys = [];
    const foreignKeyOfField = {};                                            //  tableName     table_def      /i:case-insensitive
    const schema = tableSchemaSrcOb['Create Table'].replace(/(^\s*CREATE\s+TABLE\s+`?\w+`?\s+\()([^]+?)(\).*$)/i,'$2')
                    ...
                    line => {                                           //  get rid of the KEY, CONSTRAINT, etc...
                        if ( mySqlTableLineReservedNameSet.has(         //  and only keep the column definitions
                            ...
                            if (fkMatch) {
                                ...
                                //  e.g.   ['practitioner_id'] = {    :'Practitioner',           :'id'        }
                                    foreignKeyOfField[fkColName] = { fkReferencedTableName, fkReferencedColName }
                                }
                            }
            ...
            .map(
                col => {                                          //  regex the colName  colType(colType params if present).
                    ...
                    if (! eType) {
                        throw Error(`no EDbJsType eType found for MySQL sqlType [${sqlType}] of column [${name}].`);
                    }
    ...
    const fieldsByName = schema.reduce( ( fieldsByName, field ) =>
                    Object.defineProperty(fieldsByName, field.name, {value:field}), //  returns fieldsByName
                                    {});//fieldsByName : {} initially.

    return Object.assign(
        schema.map(field =>   //  add the .fkTableName and .fkColName properties to a uni-column foreign key field.
                            Object.assign(field,
                                          foreignKeyOfField[field.name],      // may be undefined, and it's ok!
                                //  e.g.  foreignKeyOfField['practitioner_id'] = {
        ...
        { primaryKey,  uniqueKeys,  uniqueKeySet,  foreignKeys,  foreignKeyOfField,     fieldsByName });
};   //  attach .primaryKey, .uniqueKeys, .uniqueKeySet, .foreignKeys, .foreignKeyOfField and .fieldsByName props to []field/column.
self.parseTableSchema = parseMySqlTableSchema;


//endregion

[self.CURRENT_TIMESTAMP.toSqlString(),self.CURRENT_TIMESTAMP_3.toSqlString(),
