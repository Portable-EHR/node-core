...
const logger = log4js.getLogger(fileTag);
...
Function.prototype.chainProto = function (prototype=Object.getPrototypeOf(this({}))) {
    return prototype.constructor = Object.assign(this,{prototype});         // The this is This, the constructor
};
//  USAGE:
//
// const StuffProto = {};
...
//     Object.defineProperty(o, 'bibi', {get(){return this.babi;}});
//     return o;
// }
// (self.Stuff = Stuff).chainProto(/*StuffProto*/); //  might need argument StuffProto if Stuff() argument can't be {}
// //  static definitions
// Object.assign(Stuff, {
//     get A() {return 33;}                     //  BEWARE !!! : getter "this" is not comptabible with Object.assign.
// });
...
//      they've been already overriden by eiher the caller or any other proto in the proto chain not shared by the
...
Function.prototype.alsoExtends = function(alsoExtendedConstructor) {
    ...
    const addNonOverridenProp = (dominantPropsList, alsoExtendedProps, extendingOb) => {
        for (let extendedPropName of Object.keys(alsoExtendedProps)) {
            ...
            for (let dominantProps of dominantPropsList) {  //  go through all the Props of the dominant proto chain.
                if (dominantProps[extendedPropName]) {  //  if extended prop is not overriden in dominant chain:
                    foundDominant = true;
                    break;
            }}
            if (!foundDominant) {
    ...
    //  First build the collection of dominiant proto PropertyDescriptor objects in the proto chain up to common proto.
    const dominantProtoChainProps = [], dominantStaticChainProps = [];  //  For prototytpes AND constructors (static)
    ...
    addNonOverridenProp(dominantProtoChainProps, alsoExtendedProtoProps, extendingProto);
    ...
    addNonOverridenProp(dominantStaticChainProps, alsoExtendedStaticProps, extendingConstructor);
...
const isInvalidDate = date => ( !(date instanceof Date)  ||  null === date.toJSON());
...
self.TimeIntervalEnum = Object.freeze({
    nano   : 'nano',
    micro  : 'micro',
    milli  : 'milli',
    second : 'second',
...
self.toBool = v => {
    if (!v) return false;   //  covers: undefined, null, 0, false, ''.
    if (1 === v  ||  true === v) return true;
    if ('string' === typeof v) {
        if (v === '0') return false;
        if (v === '1') return true;
        v = v.toLowerCase();
    ...
    logger.error("Unable to assert as bool : ", v);
    return false;
};
/**
 *
 ...
 * @return {number}
 */
self.intFromXml = theArg => {                                                   //  Legacy function
    const util = require('util');
    if (util.isString(theArg)) return parseInt(theArg);
    if (util.isNumber(theArg)) return theArg;
    logger.error(`intFromXml : received [${theArg}], barfing`);
    throw new Error(`intFromXml : cant convert [${theArg}] `);
};

//endregion

//region string, JSON, B64, sha
...
const niceJSON = self.niceJSON = (ob) => JSON.stringify(ob, null, 4);
...
const Jwt = function Jwt(jwToken) {
    ...
    Object.assign(o, { header: JSON.parse(fromUrlB64ToString(header)),
                              ...
                              signature });
    Object.defineProperty(o, 'jwToken', {value:jwToken});   //  non-[configuable, enumerable, writable]
    return o;
};
...
self.buildFromFile = (logger, configFullFileName, classToBuild, otherParamsOfClassToBuild=[], srcJsObTest = (/*srcJsOb*/)=>'' ) => {
    let json, jsOb;
    ...
    try {   json = fs.readFileSync(configFullFileName, 'utf8'); }
    catch (e) { return logger.bailOut(`Caught error while reading [${configFullFileName}] to build [${classToBuild.name}].`, e); }

    if (!json) return logger.bailOut(`Empty JSON read from [${configFullFileName}]. Could not build [${classToBuild.name}].`);

    try {   jsOb = JSON.parse(json); }
    catch (e) { return logger.bailOut(`Caught error while parsing JSON from [${configFullFileName}] to build [${classToBuild.name}].`, e); }

    const srcJsObTestErrorMsg = srcJsObTest(jsOb);  //  Must return a non-empty Error msg on test failure, '' otherwise.
    if (srcJsObTestErrorMsg) return logger.bailOut(srcJsObTestErrorMsg);

    try {   return new classToBuild(jsOb, ...otherParamsOfClassToBuild); }
    catch (e) { return logger.bailOut(`Caught error while constructing [${classToBuild.name}] with JSON from [${configFullFileName}].`, e); }
};
...
//  a chainProto on EACH INSTANCE of wrapped error would endup with only the last wrapped Error proto be part of the
...
self.ErrorExtender = (message, ErrorSelfConstructor= ()=>ErrorExtender(message, ErrorSelfConstructor, extra, msgHead), extra={},msgHead=()=>'')=>{
    ...
    //  m is the multine flag: "When using the multiline flag, ^ also matches immediately after a line break character."
    ...
    Object.defineProperty(e, 'stack',   {configurable:true, set(stack) { stackStr = stack;},
                                get() { return stackStr ? stackStr :`${name}: ${this.verboseMsg}${errStackPerSe}`}});
...
self.Enum = function Enum(EnumConstructor=undefined) {
    ...
    //      properties and inner struct are setup.
    //
    ...
    //  funclions have over classes, which allows to perform the Enum.call(this) *after* the Object.assign.
    //
...
self.ExpectedError = function ExpectedError(msg){ return ErrorExtender(msg, ExpectedError); };
...
class LaunchParams {
    constructor({a:alias, e:environment, v:variant, f:feedFlavor, p:rootPath, c:configFileName}) {
        Object.assign(this, { alias, environment, variant, feedFlavor, rootPath, configFileName });
    }

    ensurePath() { self.makeDirIfNeeded(this.logPath, console);  }
    ...
    log(msgLoggingFnc=msg=>{console.info(msg);}) {
        for (let msg of [   `PARAMS : Starting with (a)lias           [${this.alias}]`,
                            `PARAMS : Starting with (e)nvironment     [${this.environment}]`,
                            `PARAMS : Starting with (v)ariant         [${this.variant}]`,
                            `PARAMS : Starting with (f)eed flavor     [${this.feedFlavor}]`,
                            `PARAMS : Starting with (p)ath            [${this.rootPath}]`,
                            `PARAMS : Starting with (c)onfig filename [${this.configFileName}]`, ])
            msgLoggingFnc(msg);
    }

    logLoading(msgLoggingFnc) {
        for (let msg of[`Loading from path [${this.resourcesPath}]`,
                        `Loading with      [${this.configFileName}]`,
                        `Loading with node [${this.alias}], environment [${this.environment}], variant [${this.variant}], feed flavor [${this.feedFlavor}]`])
            msgLoggingFnc(msg);
    }

    get sysopPath() { return `${this.rootPath}/${this.environment}/${this.alias}`; }
    get logPath()       { return `${this.sysopPath}/log`; }
    get resourcesPath() { return `${this.sysopPath}/resources`; }
    get configFQN()     { return `${this.resourcesPath}/${this.configFileName}`; }
    ...
    get log4jsConfig() {
        ...
        return {
            ...
            "categories": {
                ...
                "CONFIG" : {
                    "appenders": ["config", "out"], "level": "trace"
                }
            }
        };
    }
...
const isProvided = v => (undefined !== v  &&  null !== v);                              //  if null or undefined: false
const isNotProvided = v => (undefined === v  ||  null === v);                           //  if null or undefined: true

const isInvalidProvidedDate = date  =>  isProvided(date)  &&  isInvalidDate(date);      //  if null or undefined: false

const isInvalidNumber = v =>  ("number" !== typeof v);
const isInvalidProvidedNumber =  v  =>  isProvided(v)  &&  isInvalidNumber(v);          //  if null or undefined: false

const isInvalidInteger = v =>  !Number.isInteger(v);
const isInvalidProvidedInteger =  b  =>  isProvided(b)  &&  isInvalidInteger(b);        //  if null or undefined: false

const isInvalidString = v =>  ("string" !== typeof v);
const isInvalidProvidedString =  v  =>  isProvided(v)  &&  isInvalidString(v);          //  if null or undefined: false
const isInvalidOrEmptyString = v =>  isInvalidString(v)  ||  ! v;
const isInvalidOrEmptyProvidedString =  v  =>  isProvided(v)  &&  isInvalidOrEmptyString(v); //  if null or undefined: false

const isInvalidProvidedEmail = v => isProvided(v) && (isInvalidString(v) || (v && !validateEmailServerNameFormat(v))); //  if '', null or undefined: false

const isInvalidBool = v =>  ("boolean" !== typeof v);
const isInvalidProvidedBool =  b  =>  isProvided(b)  &&  isInvalidBool(b);              //  if null or undefined: false

const isInvalidEnum = (Enm, v) =>  ("object" !== typeof v)  ||  v !== Enm[v];
const isInvalidProvidedEnum =  (enm, v)  =>  isProvided(v)  &&  isInvalidEnum(enm, v);  //  if null or undefined: false

Object.assign(self, { isFunction, isProvided, isNotProvided, isInvalidProvidedDate,
    validateEmailServerNameFormat, isInvalidProvidedEmail,
    ...
    isInvalidBool, isInvalidProvidedBool, isInvalidEnum, isInvalidProvidedEnum });

const RamqNamUpTo10th = (lastName, firstName, genderAsMF, birthdate) => {
    //                                   remove all accents                 only keep alphabetical
    const strict = s => s.normalize('NFD').replace(/[\u0300-\u036f]/g, '').replace(/[^0-9a-zA-Z]/g, '').toUpperCase();
    ...
    try {
        ...
        lastName = lastName.slice(0, 3).padEnd(3, 'X');

        firstName = strict(firstName).slice(0, 1);
        ...
        if (isNaN(month)) { throw Error(`genderAsMF must be M or F.`); }
...
class Cycling {
    ...
    constructor({doOneAction=()=>{}, log=logger, cycleInterval=Infinity, enabled=true, postponedActionDelayGenerator =
        ...
        //  By extending Cycling, .doOneCycleAction() can simply be overriden and doOneAction constructor argument ignored.
        ...
        //  This provides a quick way to control Cycyling with a single parameter.
    ...
    async doOneCycleAction() {                                                              //  Candidate for Overriding
        await this._doOneAction();
    }   //  Can be overriden, for instance to provide a cycle of cycles (MobileMed spi.nao PullChunkSliceByTimeSliceBatch)
    ...
    async _interruptiblePostponedAction(delayInMs, onTimeout, onTimerInterruptedOrError) {
        ...
        const postponedAction = new Promise((fulfill, reject) => {
            ...
            //  any thrown Error would be lost. So we use this interruptable-timer core mechanism instead.
    ...
    _postponedPostErrorStart(inPausedState) {
        ...
        this._interruptiblePostponedAction(delayInMs,                           //  no await, spawn it.
            ...
            //  even if _interruptiblePostponedAction() postponedAction supplies some, omiting the following then/catch
